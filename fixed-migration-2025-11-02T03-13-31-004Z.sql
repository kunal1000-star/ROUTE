// ============================================================================
// AI STUDY ASSISTANT DATABASE SETUP - MASTER MIGRATION (FIXED)
// ============================================================================
//
...
//
// FIXED: Made fully idempotent, added IF NOT EXISTS to policies, fixed syntax errors
//
// ============================================================================
...
DROP POLICY IF EXISTS "Users can manage messages from their conversations" ON chat_messages;
DROP POLICY IF EXISTS "Users can manage their own memory" ON study_chat_memory;
DROP POLICY IF EXISTS "Users can manage their own summaries" ON memory_summaries;
DROP POLICY IF EXISTS "Users can manage their own profile" ON student_ai_profile;
DROP POLICY IF EXISTS "Users can manage their own API logs" ON api_usage_logs;
DROP POLICY IF EXISTS "All authenticated users can read prompts" ON ai_system_prompts;
DROP POLICY IF EXISTS "Service role can manage prompts" ON ai_system_prompts;
...
-- Verify all tables were created and provide completion message
DO $$
DECLARE
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE 'AI Study Assistant Migration Complete!';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Tables processed: %/%', table_count, array_length(expected_tables, 1);
    RAISE NOTICE 'System prompts: % found', prompt_count;
    RAISE NOTICE 'Extensions enabled: pgcrypto, vector, pg_stat_statements';
    RAISE NOTICE 'RLS policies: 7 tables secured';
    RAISE NOTICE 'Functions created: 5 (API logging, vector search, memory management)';
    RAISE NOTICE 'Triggers created: 4 (auto-updates and conversation tracking)';
    RAISE NOTICE 'Indexes created: 25+ for optimal performance';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Migration completed successfully!';
    RAISE NOTICE 'Database is ready for AI study assistant features.';
    RAISE NOTICE '========================================';
END $$;
