...
CREATE INDEX idx_performance_metrics_subject ON performance_metrics(subject);

-- Learning velocity indexes
CREATE INDEX idx_learning_velocity_user_id ON learning_velocity(user_id);
CREATE INDEX idx_learning_velocity_week ON learning_velocity(week_start_date);

-- Feature usage indexes
CREATE INDEX idx_feature_usage_user_id ON feature_usage_analytics(user_id);
CREATE INDEX idx_feature_usage_feature ON feature_usage_analytics(feature_name);
...
CREATE INDEX idx_performance_metrics_user_type_recorded ON performance_metrics(user_id, metric_type, recorded_at DESC);
