...
export class RealtimeDataIntegration {
  ...
  private async checkProviderHealth(provider: string): Promise<ProviderStatus> {
    try {
      ...
      const status = rateLimitTracker.getProviderStatus(provider);
  ...
  private handleUsageUpdate(payload: any): void {
    const event: RealtimeEvent = {
      ...
      data: {
        type: 'usage_event',
        provider: payload.new.provider_used,
        success: payload.new.success,
        latency: payload.new.latency_ms,
        cost: payload.new.cost_usd,
        tokens: payload.new.tokens_input + payload.new.tokens_output
      },
      source: 'database'
    };
    this.emitEvent(event);
   ...
   * Handle cache events
   */
  private handleCacheEvent(payload: any): void {
    const event: RealtimeEvent = {
      ...
      data: {
        ...
        reason: payload.new?.reason || 'unknown'
      },
      source: 'database'
    };
    this.emitEvent(event);
  ...
  private calculateUptime(provider: string): number {
    // This would typically come from a uptime tracking service
    ...
    if (recentUsage && recentUsage.length > 0) {
      ...
      return avgRequests > 10 ? 99.5 : 95.0; // Mock uptime calculation
    }
    return 0;
  }
  ...
  public getDashboardData(): Promise<{
    ...
    ]).then(([providers, usage, connections, lastUpdate]) => ({
      providers,
      usage,
      connections,
      lastUpdate
  ...
  public destroy(): void {
    ...
    console.log('🛑 Real-time Data Integration destroyed');
  }
}
