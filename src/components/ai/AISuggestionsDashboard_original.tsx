...
export function AISuggestionsDashboard({ 
  ...
  const fetchSuggestions = async (refresh = false) => {
    try {
      ...
      if (result.isHtmlResponse) {
        console.warn('⚠️ HTML response detected for suggestions:', result.error);
        setSuggestions([]);
        toast({
      ...
      if (!result.success && result.isAuthError) {
        ...
        // Show demo mode for development/testing
        toast({
          title: "Demo Mode - Authentication Required",
          description: "Using demo data. For full access, please sign up at /auth",
          variant: "destructive"
        });
  ...
  const generateNewSuggestions = async () => {
    try {
      ...
      if (result.isHtmlResponse) {
        console.warn('⚠️ HTML response detected for suggestions generation:', result.error);
        toast({
          title: "Authentication Required",
  ...
  const handleApplySuggestion = async (suggestionId: string) => {
    try {
      ...
      if (result.isHtmlResponse) {
        console.warn('⚠️ HTML response detected for apply suggestion:', result.error);
        toast({
          title: "Authentication Required",
          description: "Please log in to apply suggestions",
          variant: "destructive"
        });
        return;
      }
  ...
  const handleDismissSuggestion = async (suggestionId: string) => {
    try {
      ...
      if (result.isHtmlResponse) {
        console.warn('⚠️ HTML response detected for dismiss suggestion:', result.error);
        toast({
          title: "Authentication Required",
          description: "Please log in to dismiss suggestions",
          variant: "destructive"
        });
        return;
      }
  ...
  const handleFeedback = async (suggestionId: string, rating: number, comment?: string) => {
    try {
      ...
      if (result.isHtmlResponse) {
        console.warn('⚠️ HTML response detected for feedback:', result.error);
        toast({
          title: "Authentication Required",
